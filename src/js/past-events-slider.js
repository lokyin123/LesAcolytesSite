document.querySelectorAll("[data-past-events-slider]").forEach((slider) => {
  const track = slider.querySelector("[data-past-track]");
  const previous = slider.querySelector("[data-past-previous]");
  const next = slider.querySelector("[data-past-next]");
  const slides = Array.from(track.children);
  const indicators = Array.from(slider.querySelectorAll("[data-past-indicator]"));
  let activeIndex = 0;
