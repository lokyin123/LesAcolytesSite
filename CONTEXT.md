...
Desktop navigation links animate an ink underline from left to right on hover and keyboard focus, matching the width-grow motion used on section panels and social links; the current-page marker uses the same persistent underline. The underline is hidden in the mobile full-screen menu.
